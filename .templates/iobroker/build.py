...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Home'
  ...
  def onResize(sig, action):
    global iobrokerBuildOptions
    global currentMenuItemIndex
    mainRender(1, iobrokerBuildOptions, currentMenuItemIndex)

  iobrokerBuildOptions = []
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack ioBroker Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global iobrokerBuildOptions
    if len(iobrokerBuildOptions[selection]) > 1 and isinstance(iobrokerBuildOptions[selection][1], types.FunctionType):
      iobrokerBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, iobrokerBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(iobrokerBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(iobrokerBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(iobrokerBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'iobroker':
  main()
else:
  print("Error. '{}' Tried to run 'iobroker' config".format(currentServiceName))
